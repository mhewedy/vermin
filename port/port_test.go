...
func TestGetPortForwardArgs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "<vm port>",
			...
			want:    []string{"-L", "0.0.0.0:3000:localhost:3000"},
			wantErr: false},
		{name: "<vm port>:<local port>",
			args:    struct{ ports string }{ports: "3000:4000"},
			want:    []string{"-L", "0.0.0.0:4000:localhost:3000"},
			wantErr: false},
		{name: "<vm port>:<local port> <vmport>",
			args:    struct{ ports string }{ports: "3000:4000 8080"},
			want:    []string{"-L", "0.0.0.0:4000:localhost:3000", "-L", "0.0.0.0:8080:localhost:8080"},
			wantErr: false},
		{name: "<vmport> <vm port>:<local port>",
			args:    struct{ ports string }{ports: "8080 3000:4000"},
			want:    []string{"-L", "0.0.0.0:8080:localhost:8080", "-L", "0.0.0.0:4000:localhost:3000"},
			wantErr: false},
		{name: "<vmport> <vm port> <vm port>",
			args:    struct{ ports string }{ports: "3000 4000 5000"},
			want:    []string{"-L", "0.0.0.0:3000:localhost:3000", "-L", "0.0.0.0:4000:localhost:4000", "-L", "0.0.0.0:5000:localhost:5000"},
			wantErr: false},
		{name: "<vmport> <vm port1>-<vm port2>",
			args:    struct{ ports string }{ports: "3000 4000-4001"},
			want:    []string{"-L", "0.0.0.0:3000:localhost:3000", "-L", "0.0.0.0:4000:localhost:4000", "-L", "0.0.0.0:4001:localhost:4001"},
			wantErr: false},
		{name: "<vm port1>-<vm port2>:<local port1>-<local port2>",
			args:    struct{ ports string }{ports: "4000-4001:8080-8081"},
			want:    []string{"-L", "0.0.0.0:8080:localhost:4000", "-L", "0.0.0.0:8081:localhost:4001"},
			wantErr: false},
		{name: "<vm port1>-<vm port2>:<local port1>-<local port2> <vmport>",
			args:    struct{ ports string }{ports: "4000-4001:8080-8081 9000"},
			want:    []string{"-L", "0.0.0.0:8080:localhost:4000", "-L", "0.0.0.0:8081:localhost:4001", "-L", "0.0.0.0:9000:localhost:9000"},
			wantErr: false},
